...
def log_msg(log_widget, msg):
    log_widget.insert(tk.END, msg + "\n")
    log_widget.see(tk.END)
    log_widget.update()

def scrape_google_maps(log_widget, country, state, company_type):
    ...
    with sync_playwright() as p:
        ...
        log_msg(log_widget, f"Loading Google Maps: {url}")
        page.goto(url)
        time.sleep(8)

        log_msg(log_widget, "Scrolling to load listings...")
        for _ in range(15):
            page.mouse.wheel(0, 1000)
            time.sleep(3)

        listings = page.query_selector_all('div[role="article"]')
        log_msg(log_widget, f"Found {len(listings)} listings")

        if not listings:
            log_msg(log_widget, "No listings found - possibly selectors changed or page not loaded properly.")
        ...
        for idx, listing in enumerate(listings):
            try:
                ...
                log_msg(log_widget, f"Address: {address}")
                ...
                log_msg(log_widget, f"Entry saved.\n")
                page.go_back()
                time.sleep(5)
            except Exception as e:
                log_msg(log_widget, f"Error processing listing {idx + 1}: {e}")
                page.go_back()
                time.sleep(5)
                continue
    ...
    if results:
        ...
        log_msg(log_widget, f"Saved {len(results)} records to {filename}")
    else:
        log_msg(log_widget, "No data scraped.")
...
class App:
    def __init__(self, root):
        ...
        self.country_entry.insert(0, "USA")  # default value
        ...
        self.state_entry.insert(0, "Texas")  # default value
        ...
        self.company_entry.insert(0, "salon beauty shop")  # default value
